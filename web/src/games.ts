...
import type { Player, ResultCode, StoredGame, TakGame } from '@tak/core';
import type {
  GameBoardSize,
  GameRecord,
  GameLifecycleState,
...
import { createGameViews, recordError } from './game-views.js';
import type {
  GameError,
  GameSummary,
  GameView,
...
// The view shapes and the error taxonomy live with the view assembly in
// `game-views.ts`; this module re-exports them so consumers (`views.ts`,
// `app.ts`, `actions.ts`) keep importing the game module's vocabulary from
// one place, unchanged by the split.
export type {
  GameError,
  GameErrorCode,
  PlayerRef,
...
/** What "Your games" may be sorted by — lives with the view assembly in `game-views.ts`; `list-query.ts`'s `MY_GAMES_SCHEMA` is the one declaration of the values themselves. */
export type { GameListSort } from './game-views.js';
...
export function createGames(persistence: Persistence): Games {
   ...
   * `recordError` itself lives in `game-views.ts` with the error taxonomy.
   */
  ...
  /**
   * The one read path every list shares: the batch reads (one timestamp
   * query, one positions query, one prefs read — ADR-0010), the name cache,
   * and per-row resolution of the next mover. The per-row shaping itself
   * lives in `game-views.ts`; this orchestration decides nothing about what
   * a row shows, it only feeds the view module and fails the whole list on
   * the first read fault (a corrupt record names its game, as always).
   */
  function summariseAll(games: readonly GameRecord[], actor: SessionUser): Result<GameSummary[], GameError> {
    ...
    for (const game of games) {
      const toMovePlayer = toMovePlayerOf(game, lastPositions.value);
      if (toMovePlayer.isErr()) return err(toMovePlayer.error);
      const summary = views.summarise({
        actor,
        game,
        nameOf,
        lastMoveAt: lastMoveAt.value,
        follows,
        toMovePlayer: toMovePlayer.value,
      });
  ...
  /**
   * The next mover for one in-play game, as a core seat. The one fact a list
   * needs from a played game lives in the last move's stored position
   * (ADR-0005's read path): read from the batch `lastPositions` map, or fall
   * back to the per-record load for a game with no snapshot yet (an imported
   * record with zero live moves) — the rare case, batched never, but correct.
   */
  function toMovePlayerOf(game: GameRecord, lastPositions: ReadonlyMap<number, string>): Result<Player | null, GameError> {
    if (game.state !== 'in_play') return ok(null);
    const snapshot = lastPositions.get(game.id);
    if (snapshot === undefined) {
      return loadTakGame(game).map((tak) => tak.state.playerToMove);
    }
    return turnOf(snapshot, game.boardSize).mapErr((error) => recordError(game.id, error));
  }
  ...
  return {
    ...
    getGame(actor, gameId): Result<GameView, GameError> {
      ...
      return views.gameView({
        actor,
        game,
        record: loadedRecord.value.record,
        tak: loadedRecord.value.tak,
        nameOf: nameResolver(),
      });
    },
    ...
    listMyGames(actor: SessionUser, query: MyGamesQuery = {}): Result<GameSummary[], GameError> {
      ...
      return ok(views.sortSummaries(summaries.value, resolved.sort, resolved.direction));
    },
