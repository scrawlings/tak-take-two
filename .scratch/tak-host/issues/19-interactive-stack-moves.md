...
**2026-08-18 — Review.** Reviewed via `/code-review` across tickets 21 and 19 together. The spec axis fuzzed the state machine — 200k random traces over sizes 3–6, arbitrary interleavings of every transition, out-of-range indices and lifts — with no invariant violation. Fixed from the two reports:

- **A put-back left a stale move in the field.** Clicking the source again cleared the board selection but left the last composed PTN in the input, so *Play move* would fire a move the board no longer showed — and the hint text offers that click and Cancel as the same gesture. The adapter now empties the field whenever a transition drops the selection, and `cancel()` goes through the same path instead of duplicating it. `board-adapter.test.ts` pins both, and pins that picking a stack **up** still leaves hand-typed notation alone.
- **A stepper bound re-implemented a module rule.** `:disabled="lift <= (path.length || 1)"` restated `setLift`'s floor in markup — the untested-move-shape-knowledge that ADR-0006 exists to prevent. The adapter now exposes `liftFloor` beside `liftCeiling`.
- **`stone` had two homes**, one in the module and one on the component, because the picker assigned it directly. The picker calls `pick(kind)` and `stone` is a getter over builder state; the HTTP assertion moved with it.
