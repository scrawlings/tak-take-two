...
*`attempt` — done.* 38 sites collapsed onto `attempt(statement, fn)`; `persistence.ts` fell from 954 to 882 lines, and a failure now reads `createGame: UNIQUE constraint failed: …`. `transaction` is deliberately left hand-written: the error it reports is the closure's own, and a prefix would bury it. Three tests in `persistence.test.ts` pin the labelling and that exception.

*Not done here.* The four other architecture-review candidates listed above (Screen module, live-page module, addressed-game loader, command-path contract) are untouched — this ticket was scoped to the test seam and the two files it named. `games.ts` (1,423) and `app.ts` (916) are unchanged and remain the largest files in `web/src`.
